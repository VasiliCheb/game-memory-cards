class Card extends Phaser.GameObjects.Sprite { // создаём класс унаследованный от Phaser.GameObjects.Srite
    ...
    flip() { // создаёт метод анимации переворота карт
        this.scene.tweens.add({ // получаем доступ к объекту фейзер твин с конфигуратором (add)
            ...
            onComplete: () => { // функция в параметре создает анимацию разворачивания карты после её сжатия
                this.show(); // вызывает метод анимации разворачивания и показа карты
            }
        });
    }

    show() { // создаёт метод анимации разворачивания и показа карты
        let texture = this.opened ? 'card' + this.value : 'card'; // определяет текстуру карты. если открыта показывать картинку в другом случае показывать рубашку
    ...
    open() { // отдельный метод класса карт, открывает карту
        ...
        this.flip(); // вызывает в объекте метод анимации переворота карт (передает параметр лицевой стороны карт)
    ...
    close() { // отдельный метод класса карт, закрывает карту
        if (this.opened) { // условие выполняет проверку если карта открыта
            this.opened = false; // устанавливает флаг значение при закрытой карте
            this.flip(); // вызывает в объекте метод анимации переворота карт (передает параметр рубашки карты)
